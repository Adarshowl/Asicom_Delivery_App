...
import CancellDeliveryItem from './CancellDeliveryItem';
...
const RequestToCancel = ({ navigation }) => {
  ...
  const getAllShop = () => {
    setLoading(true);
    try {
      console.log("response axios >>> ", JSON.stringify(API_END_POINTS.Request_Cancel_List));
      console.log("response axios >>> ", JSON.stringify(userToken));

      ApiCall('get', null, API_END_POINTS.Request_Cancel_List, {
        'Content-Type': 'application/json',
        'x-access-token': userToken || userData?.remember_token,
      }).then(response => {
        // console.log("response pending >>> ", JSON.stringify(response));
        if (response?.statusCode === 200) {
          console.log("Request to camcel", JSON.stringify(response.data));
